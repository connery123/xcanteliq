<?php
    ...
    class TMDBv3{
        ...
        const _API_URL_ = "http://api.themoviedb.org/";
        ...
        public function getCastImage($name)
        {
            $person = 'query=' . urlencode($name);


            return $this->_call("search/person",$person);
        }
         ...
         * @return array
         */
        public function searchMovie($movieTitle,$year='nope'){
            if($year == 'nope')
            {
            ...
            {
                $movieTitle="query=".urlencode($movieTitle).'&year='.$year;
            }
        ...
        public function searchTv($tvtitle, $year=0)
        {
            $tvquery = "query=".urlencode($tvtitle);
            return $this->_call("search/tv",$tvquery, $this->_lang);
        }

        public function getTvInfo($id)
        {
            if(!is_int($id))
            {
                return array("_status" => 0, "_msg" => "Expected tvid to be integer. Aborting");
            }
            if(!is_numeric($id))
            {
                return array("_status" => 0, "_msg" => "Expected tvid to be numeric. Aborting");
            }


            $credits = $this->_call("tv/$id/credits","");
            $tvinfo = $this->_call("tv/$id","");
            $images = $this->_call("tv/$id/images","","");
            $images2 = $this->_call("tv/$id/images","","0");
            $videos = $this->_call("tv/$id/videos","");

            $result = array("tvinfo" => $tvinfo, "credits" => $credits, "images" => $images, "images2" => $images2, "videos" => $videos);
            return $result;
